			...
			var json={
	  		...
	  		vm.AJAX('../../../eas/c/account/detail',json,function(data){
				...
				for(let i=0;i<list.length;i++){
					str+='<li class="clr"><span class="fl"><b>'+list[i].src_dealer_name+'</b>';
					if(list[i].src_dealer_id===''){
						str+='<div>商户总数:'+list[i].src_phone+'</div></span>';
					}else str+='<div>手机号码:'+list[i].src_phone+'</div></span>';
					str+='<span class="fr">'+vm.filterNumber(list[i].free)+'</span></li>';
				}
