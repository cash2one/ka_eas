	...
	methods:{
		initPage:function(){
	  	...
	  	getList:function(){
	  		...
	  		vm.AJAX('../../../eas/c/account/openCommission',json,function(data){
	  			vm.dragStatus=3;
	  			vm.isLoad=false;
	  			setTimeout(function(){
	  				dynamics.animate(vm.c, {
	  	...
	  	filterDay:function(index){
	  		var vm=this,list=vm.list.list[index].list,arr=[],row={};
	  		for(var i in list){
	  			var m_d=vm.getDateTime(list[i].dateTime)[4];
	  			var h_m=vm.getDateTime(list[i].dateTime)[5];
	  			if(typeof row[m_d]==='undefined')row[m_d]=[];
	  			row[m_d].push({'h_m':h_m,'orderId':list[i].orderId,'phone':list[i].phone,'money':list[i].money,'type':list[i].type,'userName':list[i].userName})
	  		}
			return row;
	  	},
